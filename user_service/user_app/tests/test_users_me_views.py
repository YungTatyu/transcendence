...
class TestUsernameViewPut:
    ...
    def test_put_username_already_taken(self, api_client, create_user, create_another_user):
...
class TestAvatarViewPut:
    ...
    def test_put_avatar_user_not_found(self, api_client):
        """PUT: ユーザーが存在しない場合(エラー)"""
        response = api_client.put(
            reverse("update-avatar"), {"avatar_path": "new_avatar.jpg"}, format="multipart")
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.data["error"] == "User not found."
...
class TestAvatarViewDelete:
    ...
    def test_delete_avatar_success(self, mock_delete, mock_exists, api_client, create_user):
